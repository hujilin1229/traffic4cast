...
def train(model, train_loader, optim, device, writer, epoch, globaliter, node_pos=None):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        #Select Only the Speed Channel
        inputs = inputs[:, idx_channel*seq_len:(idx_channel+1)*seq_len, :]
        Y = Y[:, idx_channel*horizon:(idx_channel+1)*horizon, :]
        ...
        # prediction_time_channel = torch.reshape(prediction[:, :, 1:, 6:-6], (-1, 3, 1, 495, 436)) # (BZ, steps, channels, 495, 436)
        # Y_time_channel = torch.reshape(Y, (-1, 3, 1, 495, 436))

        # loss_size = torch.nn.functional.mse_loss(prediction[:, :, 1:, 6:-6][:, :, node_pos[:, 0], node_pos[:, 1]],
        #                                          Y[:, :, node_pos[:, 0], node_pos[:, 1]])
        ...
        running_loss_speed += torch.nn.functional.mse_loss(prediction_time_channel[:, :, 0, node_pos[:, 0], node_pos[:, 1]],
                                                            Y_time_channel[:, :, 0, node_pos[:, 0], node_pos[:, 1]]).item() / 255**2
        ...
        # Print statistics
        running_loss += loss_size.item() / 255**2
...
def validate(model, val_loader, device, writer, globaliter, if_testtimes=False, node_pos=None):
    ...
    with torch.no_grad():
        for i, data in enumerate(val_loader, 0):
            ...
            # Select Only the Speed Channel
            val_inputs = val_inputs[:, idx_channel*seq_len:(idx_channel+1)*seq_len, :]
            val_y = val_y[:, idx_channel*horizon:(idx_channel+1)*horizon, :]
...
if __name__ == "__main__":
    ...
    selected_channels = 1
    config['model']['in_channels'] = int((num_frames - 3) * selected_channels)  # 12*3
    config['model']['n_classes'] = int(3*selected_channels)  # 3*3
    # dimesion is (time/channels, h, w)
